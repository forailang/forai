...
const env={{
  ...
  json_query(p,l,qp,ql){{try{{const root=JSON.parse(readStr(p,l));const q=function(root,path){{var cur=[root];var segs=path.split('.');for(var i=0;i<segs.length;i++){{var seg=segs[i].trim();if(!seg)continue;var ex=seg.slice(-2)==='[]';var f=ex?seg.slice(0,-2):seg;var next=[];for(var j=0;j<cur.length;j++){{var v=cur[j];var sel;if(f===''){{sel=v}}else if(v&&typeof v==='object'&&!Array.isArray(v)&&Object.prototype.hasOwnProperty.call(v,f)){{sel=v[f]}}else{{continue}}if(ex){{if(Array.isArray(sel)){{for(var k=0;k<sel.length;k++)next.push(sel[k])}}}}else{{next.push(sel)}}}}cur=next}}return cur}};return jsToWasm(q(root,readStr(qp,ql)))}}catch(e){{return QNAN|TAG_NULL}}}},
  json_query_page(p,l,qp,ql,off,lim){{try{{const root=JSON.parse(readStr(p,l));const q=function(root,path){{var cur=[root];var segs=path.split('.');for(var i=0;i<segs.length;i++){{var seg=segs[i].trim();if(!seg)continue;var ex=seg.slice(-2)==='[]';var f=ex?seg.slice(0,-2):seg;var next=[];for(var j=0;j<cur.length;j++){{var v=cur[j];var sel;if(f===''){{sel=v}}else if(v&&typeof v==='object'&&!Array.isArray(v)&&Object.prototype.hasOwnProperty.call(v,f)){{sel=v[f]}}else{{continue}}if(ex){{if(Array.isArray(sel)){{for(var k=0;k<sel.length;k++)next.push(sel[k])}}}}else{{next.push(sel)}}}}cur=next}}return cur}};const m=q(root,readStr(qp,ql));const total=m.length;let s=Math.max(0,off|0);if(s>total)s=total;const t=Math.max(0,lim|0);return jsToWasm({{total:total,items:m.slice(s,s+t)}})}}catch(e){{return QNAN|TAG_NULL}}}},
...
var env={{
  ...
  json_query:function(p,l,qp,ql){{try{{var root=JSON.parse(readStr(p,l));var q=function(root,path){{var cur=[root];var segs=path.split('.');for(var i=0;i<segs.length;i++){{var seg=segs[i].trim();if(!seg)continue;var ex=seg.slice(-2)==='[]';var f=ex?seg.slice(0,-2):seg;var next=[];for(var j=0;j<cur.length;j++){{var v=cur[j];var sel;if(f===''){{sel=v}}else if(v&&typeof v==='object'&&!Array.isArray(v)&&Object.prototype.hasOwnProperty.call(v,f)){{sel=v[f]}}else{{continue}}if(ex){{if(Array.isArray(sel)){{for(var k=0;k<sel.length;k++)next.push(sel[k])}}}}else{{next.push(sel)}}}}cur=next}}return cur}};return jsToWasm(q(root,readStr(qp,ql)))}}catch(e){{return QNAN|TAG_NULL}}}},
  json_query_page:function(p,l,qp,ql,off,lim){{try{{var root=JSON.parse(readStr(p,l));var q=function(root,path){{var cur=[root];var segs=path.split('.');for(var i=0;i<segs.length;i++){{var seg=segs[i].trim();if(!seg)continue;var ex=seg.slice(-2)==='[]';var f=ex?seg.slice(0,-2):seg;var next=[];for(var j=0;j<cur.length;j++){{var v=cur[j];var sel;if(f===''){{sel=v}}else if(v&&typeof v==='object'&&!Array.isArray(v)&&Object.prototype.hasOwnProperty.call(v,f)){{sel=v[f]}}else{{continue}}if(ex){{if(Array.isArray(sel)){{for(var k=0;k<sel.length;k++)next.push(sel[k])}}}}else{{next.push(sel)}}}}cur=next}}return cur}};var m=q(root,readStr(qp,ql));var total=m.length;var s=Math.max(0,off|0);if(s>total)s=total;var t=Math.max(0,lim|0);return jsToWasm({{total:total,items:m.slice(s,s+t)}})}}catch(e){{return QNAN|TAG_NULL}}}},
