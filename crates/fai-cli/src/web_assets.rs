...
// Read the client-readable fai_csrf cookie so the RPC client can echo it in
// the X-Fai-Csrf header (plan 134 double-submit CSRF). '' when unset.
function faiCsrf(){{try{{var cs=document.cookie.split(';');for(var i=0;i<cs.length;i++){{var eq=cs[i].indexOf('=');if(eq>0&&cs[i].slice(0,eq).trim()==='fai_csrf')return cs[i].slice(eq+1).trim()}}}}catch(e){{}}return''}}
...
function faiScrollState(){{var d=document.scrollingElement||document.documentElement,b=Math.max(0,d.scrollHeight-d.clientHeight-(window.scrollY||0));return{{x:window.scrollX||0,y:window.scrollY||0,bottom:b,atBottom:b<=2}}}}
// Bottom-pinning is OPT-IN: only a page that renders an element with the
// `fai-follow-bottom` class (e.g. a chat thread) is re-pinned to the bottom
// after a morph when the user was at the bottom before it. Every other page
// gets a plain position restore — without the marker check, a short loading
// page counts as "at bottom" and the first content morph yanks the window
// to the bottom of the loaded page.
function faiRestoreScroll(s){{if(!s)return;var d=document.scrollingElement||document.documentElement;var follow=s.atBottom&&!!document.querySelector('.fai-follow-bottom');var y=follow?Math.max(0,d.scrollHeight-d.clientHeight):s.y;window.scrollTo(s.x,y)}}
function morphDom(root,newHtml,replaceSelf){{var scroll=faiScrollState();try{{var tmp=document.createElement('div');tmp.innerHTML=newHtml;if(replaceSelf&&root.parentNode&&tmp.childNodes.length===1){{morphNode(root,tmp.childNodes[0],root.parentNode);return}}morphChildren(root,tmp)}}finally{{faiRestoreScroll(scroll)}}}}
...
function patchAttrs(o,n){{var isF=o===document.activeElement&&o.tagName==='INPUT';var i,a,rm=[];if(o.tagName==='TEXTAREA'){{var tv=n.value;if(o.value!==tv)o.value=tv}}for(i=0;i<n.attributes.length;i++){{a=n.attributes[i];if(a.name==='value'&&o.tagName==='INPUT'){{if(o.value!==a.value)o.value=a.value;continue;}}if(o.getAttribute(a.name)!==a.value)o.setAttribute(a.name,a.value)}}for(i=0;i<o.attributes.length;i++){{if(!n.hasAttribute(o.attributes[i].name))rm.push(o.attributes[i].name)}}for(i=0;i<rm.length;i++)o.removeAttribute(rm[i])}}
const env={{
  ...
  remote_call(a,b,c,d,e,f,g,h){{const u=readStr(a,b),fn_name=readStr(c,d),ar=readStr(e,f),ha=readStr(g,h);const body=JSON.stringify({{fn:fn_name,args:JSON.parse(ar||'[]'),hash:ha}});console.log('FAI remote_call request', {{url:u,fn:fn_name,args:ar,hash:ha}});try{{const x=new XMLHttpRequest();x.open('POST',u.replace(/\/+$/,'')+'/fai/rpc',false);x.setRequestHeader('Content-Type','application/json');var _cs=faiCsrf();if(_cs)x.setRequestHeader('X-Fai-Csrf',_cs);x.send(body);const resp=JSON.parse(x.responseText);console.log('FAI remote_call response', {{fn:fn_name,ok:resp.ok,value:resp.value,error:resp.error}});if(resp.ok)return jsToWasm(resp.value);console.warn('FAI remote_call returned error', resp);return NULL_VAL}}catch(e){{console.error('FAI remote_call failed', e);return NULL_VAL}}}},
  ...
  secrets_resolve_template(){{return NULL_VAL}},
  secrets_reveal_or(){{return NULL_VAL}},
...
var env={{
  ...
  secrets_resolve_template:function(){{return NULL_VAL}},
  secrets_reveal_or:function(){{return NULL_VAL}},
