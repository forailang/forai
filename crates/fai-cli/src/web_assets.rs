...
function faiRestoreScroll(s){{if(!s)return;var d=document.scrollingElement||document.documentElement,y=s.atBottom?Math.max(0,d.scrollHeight-d.clientHeight):s.y;window.scrollTo(s.x,y)}}
