...
pub fn all_builtin_docs() -> &'static [BuiltinDoc] {
    &[
        ...
        BuiltinDoc { module: "std.secrets", name: "reveal", builtin_name: "secretsReveal", doc: "Resolve a Secret handle to its plaintext String — the single greppable audit anchor: every place plaintext enters guest memory is a reveal call. Prefer egress positions (HTTP auth headers, child process env), which never need reveal; use it only at trusted non-HTTP sinks (HMAC verification, FFI, CLI args). Raises a catchable error naming the secret and backend when the name cannot be resolved." },
        ...
        BuiltinDoc { module: "std.secrets", name: "resolveTemplate", builtin_name: "secretsResolveTemplate", doc: "Substitute $NAME and ${NAME} placeholders with resolved secret values. Only declared, resolvable names substitute; anything else (including a literal $5) passes through untouched. Reveal-class: the result may contain plaintext — send it straight to an egress sink, never into storage or agent-visible output." },
        BuiltinDoc { module: "std.secrets", name: "revealOr", builtin_name: "secretsRevealOr", doc: "Reveal a Secret's plaintext, or the default when the name cannot be resolved — for sinks that treat a missing key as 'unconfigured' rather than an error. Reveal-class: counts toward the plaintext-exposure audit." },
        // std.env
