...
fn convert_statement(stmt: &n::Statement) -> s::Statement {
    match stmt {
        ...
        // Comment statements are formatter-only and filtered out at every
        // statement-collection site before reaching here.
        n::Statement::Comment(_) => {
            unreachable!("comment statements are filtered before conversion")
        }
    }
}
