...
//!    `{ name, file, line }` (declaration site). The native runner
//!    reads it to decorate backtrace frames as `name (file:line)`;
//!    `fai build` extracts it to `<out>.dbg.json` for external tools.
//!
